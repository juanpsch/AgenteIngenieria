...
if (Test-Path $pidFile) {
    Write-Host "Ya parece levantado (.run\pids.txt existe). Corré  .\stop.ps1  primero." -ForegroundColor Yellow
    exit 1
}
...
foreach ($p in 8000, 5173) {
    if (Test-Port $p) {
        Write-Host "El puerto $p ya está en uso. Pará con  .\stop.ps1  o liberá el puerto." -ForegroundColor Yellow
        exit 1
    }
}
...
Write-Host "  Pará todo con:  .\stop.ps1" -ForegroundColor DarkGray
